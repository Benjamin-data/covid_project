...
ggplot(data = covid_19_december_2020) + 
  geom_bar(
    mapping = aes(x = Case_Reported_Date, fill=Age_Group) #Representation of the Age groups with fill
  ) +
  labs(
    x = 'Date',
    y = 'Confirmed positive cases',
    fill = 'Age group',
    title = "Confirmed positive cases (COVID-19) reported in Ontario in December 2020",
